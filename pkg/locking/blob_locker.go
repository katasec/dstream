...
func (bl *BlobLocker) AcquireLock(ctx context.Context, lockName string) (string, error) {
	...
	logger.Info("Attempting to acquire lock for blob %s", bl.lockName)
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "There is already a lease present") {
			...
			logger.Info("Lock on %s was last modified at: %v (%.2f minutes ago)", bl.lockName, lastModified.Format(time.RFC3339), lockAge.Minutes())

			if lockAge > 2*time.Minute {
				logger.Info("Lock on %s is older than 2 minutes (last modified: %v). Breaking lease...", bl.lockName, lastModified.Format(time.RFC3339))
				...
				logger.Info("Successfully acquired lock after breaking old lease for %s", bl.lockName)
				return *resp.LeaseID, nil
			}

			logger.Info("Table %s is already locked and the lock is still valid (%.2f minutes old, within 2 minute TTL). Skipping...", bl.lockName, lockAge.Minutes())
			return "", nil
		}
		return "", fmt.Errorf("failed to acquire lock for blob %s: %w", bl.lockName, err)
	}

	logger.Info("Lock acquired for blob %s with Lease ID: %s", bl.lockName, *resp.LeaseID)
	return *resp.LeaseID, nil
}
...
func (bl *BlobLocker) RenewLock(ctx context.Context, lockName string) error {
	...
	logger.Info("Lock renewed for blob %s", lockName)
	return nil
}
...
func (bl *BlobLocker) ReleaseLock(tx context.Context, lockName string, leaseID string) error {
	...
	if err != nil {
		...
		logger.Info("Lock released successfully for blob %s", bl.lockName)
	}
	return nil
}
...
func (bl *BlobLocker) StartLockRenewal(ctx context.Context, lockName string) {
	logger := logging.SetupBareLogger()
	logger.Info("Starting lock renewal for blob %s", lockName)
	go func() {
		...
		for {
			...
			case <-ticker.C:
				if err := bl.RenewLock(bl.ctx, bl.lockName); err != nil {
					logger.Error("Failed to renew lock for blob %s: %v", lockName, err)
				}
			case <-ctx.Done():
				logger.Info("Stopping lock renewal for blob %s", lockName)
				return
			}
		}
	}()
...
func (bl *BlobLocker) GetLockedTables(tableNames []string) ([]string, error) {
	...
	for _, tableName := range tableNames {
		...
		if err != nil {
			...
			logger := logging.SetupBareLogger()
			logger.Info("Failed to get properties for blob %s: %v", lockName, err)
			continue
		}
		...
		if *leaseStatus == "locked" && *leaseState == "leased" {
			logger := logging.SetupBareLogger()
			logger.Info("Table %s is locked (last modified: %v, %.2f minutes ago)",
				tableName,
			...
			if lockAge <= 2*time.Minute {
				logger.Info(" - Lock is still valid (within 2 minute TTL)")
				lockedTables = append(lockedTables, lockName)
			} else {
				logger.Info(" - Lock is stale (older than 2 minute TTL), will be broken when acquired")
			}
		}
	}
